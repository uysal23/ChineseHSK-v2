...
record_button_replacement = """                    OutlinedButton(
                    ...
                    if (hasRecording) {
                        ...
                        OutlinedButton(
                            ...
                            Text(if (isPlayingRecording) "⏹ Dinlemeyi Durdur" else "▶ Kaydımı Dinle")
                        }
                    }
"""
...
# Dashboard profile/admin support and expanded readable theme palette.
...
assert "THEME_GREEN" in progress_store.read_text(encoding="utf-8")
assert "THEME_ORANGE" in progress_store.read_text(encoding="utf-8")
assert "THEME_PINK" in progress_store.read_text(encoding="utf-8")
...
, ps)
    if light_match is None:
        raise SystemExit("ProgressStore THEME_LIGHT constant missing")
    indent = light_match.group(1)
    expanded_constants = light_match.group(0) + (
        "\n" + indent + "const val THEME_BLUE = 3"
        "\n" + indent + "const val THEME_GREEN = 4"
        "\n" + indent + "const val THEME_ORANGE = 5"
        "\n" + indent + "const val THEME_PINK = 6"
    )
    ps = ps[:light_match.start()] + expanded_constants + ps[light_match.end():]
